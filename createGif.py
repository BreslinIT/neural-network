...
INPUT_COLOR = (173, 176, 141)
OUTPUT_COLOR = (237, 81, 38)
HIDDEN_COLOR = (140, 143, 114)
BIAS_COLOR = (96, 99, 79)
...
with imageio.get_writer("2000.gif", mode="I") as writer:
    for filename in sorted([int(i.split('.')[0]) for i in os.listdir(DIRECTORY)]):
        WEIGHTS_FILE = os.path.join(DIRECTORY, str(filename)+".weights")
        img = np.zeros((IMAGE_HEIGHT,IMAGE_WIDTH,3), np.uint8)
        img.fill(215)
        cv.putText(img,str(filename),(IMAGE_WIDTH-100,IMAGE_HEIGHT-50),font,0.8,(0,0,0),1,cv.LINE_AA)
